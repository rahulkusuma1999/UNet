...
class UNet(nn.Module):
  def __init__(self):
    ...
    self.out=nn.Conv2d(     # for the output image size
        in_channels=64,
        out_channels=2,   #here you can change the out channels as per the number of segmentation we want
        kernel_size=1
    )
  ...
  def forward(self,image):
    ...
    x=self.up_trans_2(x)
    y=crop_img(x5,x)           #need to crop the image to be of expected size
    x=self.up_conv_2(torch.cat([x,y],1))


    x=self.up_trans_3(x)
    y=crop_img(x3,x)            #need to crop the image to be of expected size
    x=self.up_conv_3(torch.cat([x,y],1))


    x=self.up_trans_4(x)
    y=crop_img(x1,x)           #need to crop the image to be of expected size
